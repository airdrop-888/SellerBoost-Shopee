...
![Version](https://img.shields.io/badge/Version-1.0.0%20PRO-success?style=for-the-badge)
...
Software ini akan berjalan otomatis di PC/RDP Anda 24/7. Tidak hanya sekadar mengklik, bot ini dibekali **Kecerdasan Buatan (Smart Cooldown)** untuk membaca server Shopee agar 100% aman dari Banned/Shadowban.
...
*   🧠 **Smart Cooldown Tracker (Anti-Spam)** — Bot mampu membaca sisa waktu dari server Shopee secara *real-time*. Jika slot penuh, bot otomatis tidur dan akan bangun tepat di detik saat tombol boost siap ditekan lagi.
*   🛡️ **Human-Like Delay** — Dilengkapi dengan algoritma *random delay* (jeda acak 3-6 detik) per aksi. Aktivitas bot akan terlihat persis seperti manusia asli di mata sistem keamanan Shopee.
*   🎯 **Dynamic Smart Selection** — Otomatis menganalisa daftar produk *Live* Anda dan selalu memprioritaskan produk berbeda yang belum diboost.
...
*(Tambahkan gambar screenshot terminal aplikasi .exe Anda di sini untuk meyakinkan pembeli)*
![Preview Terminal](https://via.placeholder.com/800x400/000000/00FF00?text=Premium+Terminal+UI:+Smart+Countdown+%26+Multi-Account)
...
1. **Buka folder** hasil download.
2. Buka file `cookies.txt` menggunakan Notepad, lalu **masukkan Cookie akun Shopee Seller Anda** (1 Baris = 1 Akun Toko).
3. **Klik 2x (Double Click)** pada aplikasi `ShopeeBoost.exe`.
